...
export default class GanttChart extends LightningElement {
  ...
  setStartDate(_startDate) {
    if (_startDate instanceof Date && !isNaN(_startDate)) {
      ...
      this.startDateUTC = window.moment(this.startDate).utc().valueOf() + '';
      this.formattedStartDate = this.startDate.toLocaleDateString('ja-JP', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit'
      });
  ...
  setDateHeaders() {
    this.endDate = window
      ...
      .toDate();
    this.endDateUTC = window.moment(this.endDate).utc().valueOf() - window.moment(this.endDate).utcOffset() * 60 * 1000 + '';
    this.formattedEndDate = this.endDate.toLocaleDateString('ja-JP', {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit'
    });
    ...
    for (
      ...
      let d = date.format('D');
      let m = date.format('M');
      let dateLabel = ('0' + m).slice(-2) + '/' + ('0' + d).slice(-2);
      ...
      if (this.view.slotSize > 1) {
        let end = window.moment(date).add(this.view.slotSize - 1, 'days');
        day.end = end.toDate();
      } else {
        day.end = date.toDate();
        ...
        day.dayName = formatter.format(date.toDate()).slice(0, 1);
        if (date.day() === 0) {
