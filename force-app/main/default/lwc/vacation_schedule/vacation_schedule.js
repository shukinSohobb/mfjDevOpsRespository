...
import DriverSchedule_OBJECT from '@salesforce/schema/DriverSchedule__c';
import DriverMaster_DeliveryDestinationTeam_FIELD from '@salesforce/schema/DriverMaster__c.DeliveryDestinationTeam__c';
import DriverSchedule_WorkStatus_FIELD from '@salesforce/schema/DriverSchedule__c.WorkStatus__c';
import { getPicklistValues, getObjectInfo } from 'lightning/uiObjectInfoApi';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import getDrive from '@salesforce/apex/DriverScheduleController.getDrive';
...
export default class Work_Schedule extends LightningElement {
  ...
  @wire(getObjectInfo, { objectApiName: DriverSchedule_OBJECT })
  driverScheduleObjectInfo;

  @wire(getPicklistValues, {
    recordTypeId: '$driverScheduleObjectInfo.data.defaultRecordTypeId',
    fieldApiName: DriverSchedule_WorkStatus_FIELD
  })
  workStatusOptions;

  get _workStatusOptions() {
    let select = [];
    // select.push({
    //     "attributes": null,
    //     "label": "",
    ...
    //     "value": ""
    // });
    this.workStatusOptions.data.values.forEach((element) => {
      // if (element.value != 'Dispatched') {
      select.push(element);
      // }
    });
    return select;
