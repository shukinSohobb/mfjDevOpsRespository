...
export default class GanttChartResource extends LightningElement {
  ...
  calcLabelStyle(allocation) {
    ...
    let right =
      (totalSlots - (allocation.right + 1)) / totalSlots < 0 ? 0 : (totalSlots - (allocation.right + 1)) / totalSlots;
    // let styles = ['left: calc(' + left * 100 + '% + 15px)', 'right: calc(' + right * 100 + '% + 30px)'];
    let styles = ['left: calc(' + left * 100 + '% + 15px)'];
  ...
  setProjects() {
    ...
    Object.keys(self._resource.allocationsByProject).forEach((projectId) => {
      ...
      */
      ...
      self.resource.allocationsByProject[projectId].forEach((tempAllocation) => {
  ...
  editAllocationModalSuccess() {
    const startDate = new Date(this.getYYYYMMDD(this.editAllocationData.startDate));
    const endDate = new Date(this.getYYYYMMDD(this.editAllocationData.endDate));
    this._saveAllocation({
